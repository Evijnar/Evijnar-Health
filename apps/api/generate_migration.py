"""
Helper script to generate initial migration from SQLAlchemy models.
This generates the migration file content based on models.database.
"""
...
def generate_migration_file():
    """Generate the initial migration file with all table creation ops."""

    migration_id = datetime.utcnow().strftime("%Y%m%d%H%M%S")
    filename = f"/home/vinjai/Evijnar/apps/api/alembic/versions/{migration_id}_initial_migration.py"

    # Start with migration header
    content = '''"""Initial migration: create all database tables
